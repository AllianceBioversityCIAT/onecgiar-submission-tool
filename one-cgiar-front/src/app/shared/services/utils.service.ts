...
export class UtilsService {
  ...
  constructor(private router: Router, private _initiativesService: InitiativesService, private _authService: AuthService, private _interactionsService: InteractionsService) {}
  ...
  goToEditToc() {
    ...
    this._initiativesService
      .getProposalTocByInitiativeId()
      .pipe(map(res => res?.response?.fullInitiativeToc?.toc_id))
      .subscribe(toc_id => {
        console.log(toc_id);
        console.log(this._authService?.userValue?.id);
        this._initiativesService.authTocToken(this._authService?.userValue?.id).subscribe(
          token => {

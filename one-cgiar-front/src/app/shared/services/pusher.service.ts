...
import { PusherBlocked } from '../components/navbar/cgiar-logo-svg/variables/pusher-blocked-routes';
...
export class PusherService {
  ...
  validaeFirstUserToEdit() {
    if (!this.presenceChannel?.members) return;
    ...
    Object.keys(members).map((item) => {
      ...
      membersList.push({
        userId: item,
        date: initiativeRoles?.length
          ? initiativeRoles[0]?.name !== 'Guest'
            ? date
            : oldDate
          : generalRoles?.length
          ? generalRoles[0]?.name == 'Admin'
            ? date
            : oldDate
          : oldDate,
        role:
          this._initiativesService.initiative.status == 'Approved'
            ? 'Guest'
            : initiativeRoles?.length
            ? initiativeRoles[0]?.name
            : generalRoles?.length
            ? generalRoles[0]?.name == 'Admin'
              ? generalRoles[0]?.name
              : 'Guest'
    ...
    if (this.firstUser && this.secondUser) {
      ...
      this.router
        .navigateByUrl(
          `/initiatives/${this._initiativesService.initiative.id}/stages/${this._initiativesService.initiative.exactStageName}`
        )
        .then(() => {
          setTimeout(() => {
    ...
    return (
      membersList[0]?.userId == myID ||
      (!this._initiativesService.initiative.userRoleName &&
        !this._authService.lsUserRoles.name) ||
      (!this._initiativesService.initiative.userRoleName &&
        this._authService.lsUserRoles.name == 'Guest') ||
      this._initiativesService.initiative.readonly
    );
  }
  ...
  listenTocChange(sectionName: string, callback, subItemId?: string | number) {
    ...
    this.instancePusher();
    const channelName = `${sectionName}-${
      this._initiativesService.initiative.id
    }${subItemId ? `-${subItemId}` : ``}`;
  ...
  start(OSTRoute: string, userId, initiativeId) {
    ...
    if (this.beforeRoute)
      this.pusher.unsubscribe('presence-ost' + this.beforeRoute);
  ...
  updateStatus(tocStatus) {
    this.http
      .post(`${environment.apiUrl}/auth/pusher/update`, {
        tocStatus: tocStatus,
      })
      .subscribe((data) => {});
  }

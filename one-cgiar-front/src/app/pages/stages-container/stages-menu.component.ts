...
export class StagesMenuComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.sectionsList = this.router.routerState.snapshot.url
      .substring(this.router.routerState.snapshot.url.indexOf('stages/'))
      .split('/');
    let testi = 1;
    this.router.events.subscribe((event: NavigationEvent) => {
      if (event instanceof NavigationStart) {
        this.sectionsList = event.url
          .substring(event.url.indexOf('stages/'))
          .split('/');
        this._dataControlService.breadcrumbItemTwo =
          event?.url.indexOf('work-package') !== -1
            ? this._dataControlService.breadcrumbItemTwo
            : '';
      }
    });
  ...
  validateCurrentStageChange(stageName) {
    ...
    if (this.currentStageAux != stageName && this.currentStageAux != '') {
      this.router
        .navigateByUrl(
          `/initiatives/${this.initiativesSvc.initiative.id}/stages/${this.initiativesSvc.initiative.exactStageName}`
        )
        .then(() => {
          setTimeout(() => {
  ...
  activedRouteEvent(stagesList: []) {
    this.activatedRoute.params.subscribe((resp) => {
      ...
      stagesList.map((stageItem: any) => {
        stageItem.stageNameKebabCase = this._utilsService.convertToKebabCase(
          stageItem?.description
        );
      });

      let currentStage: any = stagesList.find(
        (stageItem: any) => stageItem?.stageNameKebabCase == resp?.stageName
      );
      // console.log(currentStage)
      // console.log(currentStage?.id)
      // console.log("Change initiative=> " , currentStage?.id);
      this.initiativesSvc.initiative.stageName =
        currentStage?.stageNameKebabCase != 'pre-concept'
          ? 'proposal'
          : currentStage.stageNameKebabCase;
      this.initiativesSvc.initiative.exactStageName =
        currentStage?.stageNameKebabCase;
      ...
      forkJoin([promise1, promise2]).subscribe(
        (val) => {
          // console.log(val)
          const sucP1 = val[0];
          ...
          this.initiativesSvc
            .getSubmission(
              this.initiativesSvc.initiative.id,
              this.initiativesSvc.initiative.stageId
            )
            .subscribe(
              (res) => {
  ...
  getInitiativeReadOnlyValidation(initiative) {
    ...
    this.initiativesSvc
      .getRolefromInitiativeById(this.initiativesSvc.initiative.id)
      .subscribe(
        (resp) => {
          //  console.log(resp)
          // console.log(resp?.response?.roles[0]?.name)
          this.initiativesSvc.initiative.userRoleName =
            resp?.response?.roles[0]?.name;
          this.initiativesSvc.initiative.userRoleId =
            resp?.response?.roles[0]?.roleId;
          ...
          let validations = () => {
            ...
            if (
              ...
              resp?.response?.roles[0]?.roleId != undefined &&
              initiative?.status == 'Editing'
            )
              return false;
            return true;
          };
  ...
  validateAllSections() {
    ...
    this.initiativesSvc
      ...
      .subscribe((resp) => {
        if (this.initiativesSvc.initiative.stageId == 4)
          this._dataControlService.isdcFeedbackValidation =
            resp?.response.isdcFeedBack;
        ...
        Object.keys(resp?.response).map((key) => {
          ...
          let result = this._dataControlService?.userMenu
          ...
          subSectionsToMap.map((item) => {
            let menuSubsections = result.subsections.find(
              (subSeItem) => subSeItem.subSectionId == item.subSectionId
            );
            if (menuSubsections)
            ...
            item.dinamicList?.map((resp) => {
              if (
                !menuSubsections?.dynamicList?.find(
                  (dynamicItem) => dynamicItem.id == resp.impact_area_id
                )
              )
                return false;
              menuSubsections.dynamicList.find(
                (dynamicItem) => dynamicItem.id == resp.impact_area_id
              ).fieldsCompleted = resp.validation;
            });
          });
        });
      });

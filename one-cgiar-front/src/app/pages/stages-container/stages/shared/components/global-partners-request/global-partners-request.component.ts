...
export class GlobalPartnersRequestComponent implements OnInit {
  ...
  constructor(public _initiativesService: InitiativesService, public dialog: MatDialog) {}
    ...
    for (var i in this.institutionsTypesSavedList) {
      if (this.institutionsTypesSavedList[i].active !== false) {
        if (!lookupObject[this.institutionsTypesSavedList[i]['name']]?.cont) {
          lookupObject[this.institutionsTypesSavedList[i]['name']] = {
            cont: 0,
            name: this.institutionsTypesSavedList[i]?.name,
            code: this.institutionsTypesSavedList[i]?.code
          };
        }
  ...
  onSelectOption(option: any) {
    ...
    const institutionsTypeFound = this.institutionsTypes.find((savedItem: any) => savedItem?.code == itemFinded?.institutionTypeId);
    if (institutionsTypeFound) institutionsTypeFound.disabled = false;
    //formas de borrar
    if (itemFinded) {

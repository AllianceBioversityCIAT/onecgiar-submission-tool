...
export class ConceptValidation extends InitiativeStageHandler {
  ...
  public async isNarrtvComplete() {
    try {
      // get narratives data
      const narrativesData = await this.queryRunner.query(
        `SELECT challenge, objectives, results, highlights FROM narratives WHERE initvStgId = ${this.initvStgId_}`
      );
  ...
  public async isTOCComplete() {
    try {
      // get tocs data
      const tocs = await this.queryRunner.query(
        `SELECT id FROM tocs WHERE initvStgId = ${this.initvStgId_}`
      );
      // get tocs files data
      const files = await this.queryRunner.query(
        `SELECT id FROM files WHERE tocsId =(SELECT id FROM tocs WHERE initvStgId = ${this.initvStgId_})`
      );
  ...
  public async isWPComplete() {
    try {
      ...
      const workPackages = await this.queryRunner.query(
        `SELECT name, results, pathway_content, acronym FROM work_packages WHERE initvStgId = ${this.initvStgId_} AND active = 1`
      );
      // get work packages regions data
      const regions = await this.queryRunner.query(
        `SELECT region_id FROM regions_by_work_packages WHERE wrkPkgId = (SELECT id FROM work_packages WHERE initvStgId = ${this.initvStgId_} AND active = 1 LIMIT 1)`
      );
      // get work packages countries data
      const countries = await this.queryRunner.query(
        `SELECT country_id FROM countries_by_work_packages WHERE wrkPkgId = (SELECT id FROM work_packages WHERE initvStgId = ${this.initvStgId_} AND active = 1 LIMIT 1)`
      );
  ...
  public async isKPComplete() {
    try {
      ...
      const keyPartners = await this.queryRunner.query(
        `SELECT key_partner_id, key_partner_name, description FROM key_partners WHERE partnershipsId =(SELECT id FROM partnerships WHERE initvStgId = ${this.initvStgId_})`
      );
      // validate if any field is empty or null
  ...
  async validateCompletness() {
    ...
    const currentInitvStg = await this.initvStgRepo.findOne({
      where: {id: this.initvStgId_},

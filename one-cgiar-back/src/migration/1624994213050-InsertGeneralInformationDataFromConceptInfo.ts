...
export class InsertGeneralInformationDataFromConceptInfo1624994213050
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    // get concept information data
    const conceptInfo = await queryRunner.query(
      `SELECT name, action_area_description, action_area_id, initvStgId FROM concept_info`
    );

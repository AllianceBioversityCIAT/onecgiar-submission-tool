...
export class InsertWorkPackageImpactAreas1621948449651
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const initiaitivesId = impactSheet
      .getColumn('A')
      .values.slice(2, impactSheet.getColumn('A').values.length);
    // extract impact names column
    const impactNames = impactSheet
      .getColumn('B')
      .values.slice(2, impactSheet.getColumn('B').values.length);
    // extract impact description column
    const impactDescp = impactSheet
      .getColumn('C')
      .values.slice(2, impactSheet.getColumn('C').values.length);

    // get work packages from initiatives
    const workPackages = await queryRunner.query(
      `SELECT * FROM work_packages WHERE initvStgId IN (SELECT id FROM initiatives_by_stages WHERE initiativeId IN (${initiaitivesId}) AND stageId = 2 )`
    );
    ...
    impactNames.forEach((impactName, i) => {
      ...
      let area = impactAreas.data.find(
        (IA) =>
          IA.name.toLowerCase().indexOf(impactName.toString().toLowerCase()) !=
          -1
      );
      const pB = {
    ...
    const grouped = _.mapValues(_.groupBy(pBArray, 'initiativeId'), (clist) =>
      clist.map((pB_) => _.omit(pB_, 'initiativeId'))
    );

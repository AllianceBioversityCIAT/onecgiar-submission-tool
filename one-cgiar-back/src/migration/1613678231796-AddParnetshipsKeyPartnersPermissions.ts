...
export class AddParnetshipsKeyPartnersPermissions1613678231796 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        const IO_role_ = await getRepository(Roles).findOne({ where: { acronym: 'SGD' } })
        const IC_role_ = await getRepository(Roles).findOne({ where: { acronym: 'PI' } })

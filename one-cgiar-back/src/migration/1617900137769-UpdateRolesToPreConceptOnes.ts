...
export class UpdateRolesToPreConceptOnes1617900137769
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const co_lead = await getRepository(Roles).findOne({
      where: {acronym: 'PI'}
    });
    const guest = await getRepository(Roles).findOne({
      where: {acronym: 'GUEST'}
    });
    ...
    lead.description =
      'Science Group Directors/Designated (SGD) / Initiative Design Team';
    ...
    let updatedRoles = await rolesRepository.save([
      lead,
      co_lead,
      guest,
      coordinator,

...
//  --- 1621436229812
// 1621451356424
export class InsertWorPackagesPreConceptSBTConcept1621451356424
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (let index = 2; index <= wSheet.rowCount; index++) {
      ...
      const initvId = wb
        .getCellInRowByColumnHeader(wSheet, index, 'initiative_id')
        .value.toString();
      let tlt = wb
        .getCellInRowByColumnHeader(wSheet, index, 'title')
        .value.toString();
      const acronym = `Work Package ${wb
        .getCellInRowByColumnHeader(wSheet, index, 'Work Package')
        .value.toString()}`; // wb.getCellInRowByColumnHeader(wSheet, index, 'Work Package').value
      const name = tlt;
      const pathwayContent = wb
        .getCellInRowByColumnHeader(wSheet, index, 'Main focus')
        .value.toString();
      const results = wb
        .getCellInRowByColumnHeader(wSheet, index, 'Outcomes')
        .value.toString();
      // const wpId = wb.getCellInRowByColumnHeader(wSheet, index, 'Work Package').value.toString();

      // get initiative by stage
      const intvStg = await initvStageRepo.findOne({
        where: {initiative: initvId}
      });

...
export class InsertPreConceptLeadCoLead1620664266932 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        try {
             ...
             */
            const initvStgs = await initvStageRepo.find({ relations: ['initiative', 'stage'] });
            ...
            for (let index = 0; index < initvStgs.length; index++) {
                ...
                */
                if (element) {
                    let excelLead = wb.getCellInRowByColumnHeader(wSheet, element.id + 1, 'PCF002_InitLeadName').value;
                    let excelLeadEmail = wb.getCellInRowByColumnHeader(wSheet, element.id + 1, 'PCF003_InitLeadEmail').value;
                    /**
                     * check if user exists 
                     */
                    user = await this.parseUser(excelLead.toString(), excelLeadEmail.toString());
                ...
                */
                if (element) {
                    let excelLead = wb.getCellInRowByColumnHeader(wSheet, element.id + 1, 'PCF004_InitCoLeadNam').value;
                    let excelLeadEmail = wb.getCellInRowByColumnHeader(wSheet, element.id + 1, 'PCF005_InitCoEmail').value;
                    /**
                     * check if user exists 
                     */
                    user = await this.parseUser(excelLead.toString(), excelLeadEmail.toString());

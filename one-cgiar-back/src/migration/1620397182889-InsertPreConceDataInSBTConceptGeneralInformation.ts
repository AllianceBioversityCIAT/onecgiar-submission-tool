...
export class InsertPreConceDataInSBTConceptGeneralInformation1620397182889
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
       ...
       * Get concept stage
       */
      const concept = await stageRepo.findOne({
      ...
      for (let index = 0; index < initiatives.length; index++) {
         ...
         * Assign action area
         */
        let excelAA = wb.getCellInRowByColumnHeader(
          wSheet,
          index + 1,
          'AA_Code'
        )
          ? wb.getCellInRowByColumnHeader(wSheet, index + 1, 'AA_Code').value
          : null;
        if (excelAA) {
          const clarisaAA = actionAreas.find((AA) =>
            excelAA.toString().includes(AA.acronym)
          );
          newConceptInfo.action_area_description = clarisaAA
            ? clarisaAA.name
            : null;

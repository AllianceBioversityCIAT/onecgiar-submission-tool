...
export class InsertWorkPackageCountries1621803755153 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        */
        ...
        const countriesContributionSheet = await wb.readWorkSheet('countries_contribution');
        const countriesSheet = await wb.readWorkSheet('countries');

        // extrac countries initaitives id column
        const countriesInitiativesId = countriesContributionSheet.getColumn('A').values.slice(2, countriesContributionSheet.getColumn('A').values.length);
        // extrac countries names column
        const countriesNames = countriesContributionSheet.getColumn('D').values.slice(2, countriesContributionSheet.getColumn('D').values.length);


        // extrac countries names column
        const cNames = countriesSheet.getColumn('C').values.slice(2, countriesSheet.getColumn('C').values.length);
        // extrac countries code column
        const countryCode = countriesSheet.getColumn('A').values.slice(2, countriesSheet.getColumn('A').values.length);


        // get work packages from initiatives
        const workPackages = await queryRunner.query(`SELECT * FROM work_packages WHERE initvStgId IN (SELECT id FROM initiatives_by_stages WHERE initiativeId IN (${countriesInitiativesId}) AND stageId = 2 )`);
        ...
        let grouped = _.mapValues(_.groupBy(countriesArray, 'initiativeId'),

...
export const createRole = async (req: Request, res: Response) => {
  ...
  try {
    ...
    } else res.json({msg: 'Role created', data: createdRole});
  } catch (error) {
    console.log(error);
    if (
      error instanceof QueryFailedError ||
...
export const createPermission = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rolesDB && rolesDB.length > 0) permission.roles = rolesDB;
    else return res.status(400).json({data: rolesDB, msg: 'None role found'});
